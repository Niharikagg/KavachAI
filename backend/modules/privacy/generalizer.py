...
_HEALTH_CATEGORY_KEYWORDS = {
    "neurological": "neurological condition",
    "autoimmune": "autoimmune condition",
}


def generalize_health(value: str) -> dict:
    """Reduce a specific health value to a useful, less identifying category."""
    normalized_value = value.lower()
    for keyword, category in _HEALTH_CATEGORY_KEYWORDS.items():
        if keyword in normalized_value:
            return {"new_value": category, "info_loss_cost": 0.6}
    return {"new_value": "health condition", "info_loss_cost": 0.6}
...
def get_transformation(attribute: dict) -> dict | None:
    ...
    if attr_type in _PII_TYPES:
        ...
        if specificity >= 0.9:
            if attr_type == "HEALTH":
                result = generalize_health(attr_value)
            else:
